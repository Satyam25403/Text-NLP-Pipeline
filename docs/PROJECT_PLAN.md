...
│   └── index-schema.json              # Full index schema (fields, vector config, semantic, scoring)
│   # NOTE: indexer.json and skillset.json are intentionally absent.
│   #   fn-index-refresh uses SearchClient.mergeOrUploadDocuments() (push API).
│   #   Pull-based indexers and skillsets are not used in this pipeline.
│
